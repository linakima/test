...
BBCode.prototype = {
  ...
  construct: function(textarea) {
    ...
    this.addTag(
      '_quoter',
      function() { return '[quote="'+th.quoter+'"]' + th.qpost },
      '[/quote]\n',
      null,
      null,
    ...
    //   var tr = $(e.target).closest('#topic_main tbody');
    //   if (tr[0]) {
    //     BBCode.prototype.qpost = '[qpost=' + (tr[0].id || '').split('_')[1]  + ']' || '';
    //     //BBCode.prototype.MSG_SELECT_AUTHOR = $.trim($(tr[0]).find('p.nick').text());
    //   }
    // });
  },
  ...
  onclickPoster: function(name) {
	...
	onclickQuoteSel: function() {
		var sel = this.getSelection()[0];
		if (sel) {
			this.insertAtCursor('[quote'+ (this.quoter ? '="'+this.quoter+'"' : '') +']' + this.qpost + sel + '[/quote]\n');
		}
		else {
  ...
  getSelection: function() {
    ...
    // set quoter username & quote post id
    if (text) {
      var $tr = $(w.getSelection().anchorNode.parentNode).closest('tr');
      var quoter = $tr.find('.nick > a').text();
      var qpost = $tr.find('a').first().attr('name');
      this.quoter = quoter ? quoter : '';
      this.qpost = qpost ? '[qpost=' + qpost + ']' : '';
    }

    return [text.trim(), range];
  },
